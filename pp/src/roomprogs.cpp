...
int doit(CHAR_DATA *ch, char *func, char *arg, char *, char *, char *, room_prog_var *);
			...
			while (strLine.find("&(") != std::string::npos)
			{
				...
				if (is_number(TempString.c_str()))
				{
					if (atoi(TempString.c_str()) == 0)
						strLine.replace(first_index, index-first_index, argument);
					else
					{
						...
						strLine.replace(first_index, index-first_index, ThisArgument);
					}
				}
...
reval (CHAR_DATA * ch, char *arg)
{
	...
	*rbuf = '\0';
	...
	else
	{
		...
		if (sarg[0] = '(')
		{
			while (sarg[var_int_test] != ')' || sarg[var_int_test] != '\0' || sarg[var_int_test] != '<' || sarg[var_int_test] != '>' || sarg[var_int_test] != '=' || sarg[var_int_test] != '!')
			{
				var_buf_test.push_back(sarg[var_int_test++]);
			}

			if (sarg[var_int_test] == ')' || sarg[var_int_test] == '\0')
				return;

			char test_char = sarg[var_int_test++];

			if (sarg[var_int_test] == ')' || sarg[var_int_test] == '\0')
				return;

			while (sarg[var_int_test] != ')' || sarg[var_int_test] != '\0')
			{
				var_buf_rhs.push_back(sarg[var_int_test++]);
			}

			if (var_buf_rhs.empty() || var_buf_test.empty())
				return;
			...
			case '=':
				equals = true;
			case '!':
				if (!var_buf_test.compare(var_buf_rhs))
				{
					if (!equals)
						ifin[nNest] = 1;
					return;
			...
			case '<':
				if (is_number(var_buf_test.c_str()) && is_number(var_buf_rhs.c_str()))
				{
					if (atoi(var_buf_rhs.c_str()) > atoi(var_buf_test.c_str()))
						return;
			...
			case '>':
				if (is_number(var_buf_test.c_str()) && is_number(var_buf_rhs.c_str()))
				{
					if (atoi(var_buf_test.c_str()) > atoi(var_buf_rhs.c_str()))
						return;
