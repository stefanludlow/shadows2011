...
charstat (CHAR_DATA * ch, char *name, bool bPCsOnly)
{
	...
	for (af = k->hour_affects; af; af = next_affect)
	{
		...
		else if (af->type >= MAGIC_SKILL_GAIN_STOP && af->type < MAGIC_SPELL_GAIN_STOP) {
			sprintf (buf, "00000   Skill Gain Timer.\n");
			send_to_char (buf, ch);
			continue;
		}
...
do_set (CHAR_DATA * ch, char *argument, int cmd)
{
	...
	if (!str_cmp (subcmd, "effort"))
	{
		...
		else
		{
			...
			if (ch->fight_percentage == 100)
				sprintf(buf+strlen(buf), "#F");
			else if (ch->fight_percentage <= 20)
...
void
do_toggle (CHAR_DATA * ch, char *argument, int cmd)
{
	CHAR_DATA *tch;
	int ind = -1;
	...
	if (args.pop() == "") {
		send_to_char ("Toggle whose flags?\n", ch);
		return;
	}

	if (!(tch = get_char_room_vis (ch, args.last().c_str())))
	{
		send_to_char ("They aren't here.\n", ch);
		return;
	}
	...
	if (args.pop() == "") {
		send_to_char("What flag do you want to toggle?\n", ch);
		return;
	}

	if ((ind = index_lookup (action_bits, args.last().c_str())) != -1)
	{ 
		if (!strcmp (args.last().c_str(), "stayput")) {
			send_to_char("See HELP STAYPUT for usage of this functionality.\n", ch);
			return;
		}
		if (IS_SET (tch->act, 1 << ind)) {
			tempStream << "This mobile has had the flag " << action_bits[ind] << " toggled OFF.\n";
			tch->act &= ~(1 << ind);
		}
		else {
			tempStream << "This mobile has had the flag " << action_bits[ind] << " toggled ON.\n";
			tch->act |= (1 << ind);
		}
	}
	else {
	...
	send_to_char(tempStream.str().c_str(), ch);
}
