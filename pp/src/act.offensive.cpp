...
projectile_shield_block (CHAR_DATA * ch, int result)
{
	...
	if ((shield_obj = get_equip (ch, WEAR_SHIELD)) && number (1, 20) <= ch->dex)
	{
		...
		if (roll <= (ch->skills[SKILL_BLOCK] + shield_obj->o.od.value[2]) && rollCoverage <= shield_obj->o.od.value[2])
		{
			return 1;
		}
	}
...
do_guard (CHAR_DATA * ch, char *argument, int cmd)
{
	...
	if (get_affect(ch,MAGIC_HIDDEN))
	{
		send_to_char ("You cannot guard while hidden.\n", ch);
		return;
	}
	...
	{
		send_to_char ("You cannot guard someone who is hiding.\n", ch);
		return;
	}
